...
export function buildPlugins({
  ...
  const plugins: Configuration["plugins"] = [
    new HtmlWebpackPlugin({
      // плагин отвечающий за добавление сбандленных скриптов
    ...
    new DefinePlugin({
      // данный плагин нужен для импортирования в приложения различных внешних переменных
  ...
  if (isProd) {
    plugins.push(
      new MiniCssExtractPlugin({
        // данный плагин нужен для бандла всех css стилей в сss файлы
    ...
    plugins.push(
      new CopyPlugin({
        // данный плагин нужен для копирования статических файлов в итоговую сборку без изменений
        patterns: [
          {
            from: path.resolve(paths.public, "locales"),
            to: path.resolve(paths.output, "locales"),
          }, // здесь указывается откуда статические файлы копируются и куда
          {
            from: path.resolve(paths.public, "img"),
